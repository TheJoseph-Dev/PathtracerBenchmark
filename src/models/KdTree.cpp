...
std::array<int, 2> KdTree::SplitSAH(const AABB& bounds, int l, int r, uint32_t axis) {
    ...
    auto ternary_partition = [this, axis](int l, int r, float splitPos) -> std::array<int, 2> {
        /*
        auto lEnd = std::partition(triangles.begin() + l, triangles.begin() + r, [&](const Triangle& tri) {
            return tri.bbox.max[axis] < splitPos;
        });

        auto rBegin = std::partition(lEnd, triangles.begin() + r, [&](const Triangle& tri) {
            return tri.bbox.min[axis] < splitPos;
        });
        */
        int lEnd = l, rBegin = r;
