...
SPIRV::SPIRV(const VulkanContext& vkCtx, const Pathtracer::Config& pathtracerConfig) : ComputeBackend(vkCtx, pathtracerConfig) {
...
VkCommandBuffer SPIRV::beginSingleTimeCommands() const {
...
void SPIRV::endSingleTimeCommands(VkCommandBuffer commandBuffer) const {
...
void SPIRV::create2DNearestImageSampler() {
...
void SPIRV::createImage2D(uint32_t width, uint32_t height) {
...
void SPIRV::createPathtracerImages() {
...
void SPIRV::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void SPIRV::createSSBO(const VkBuffer& buffer, uint32_t binding) {
...
void SPIRV::createDescriptorSetLayout() {
...
void SPIRV::createComputePipeline() {
...
void SPIRV::init(const SceneData& sceneData) {
	const uint32_t WIDTH = this->pathtracerConfig.GetResolution().x, HEIGHT = this->pathtracerConfig.GetResolution().y;
...
void SPIRV::dispatch(const DispatchConext& dispatchCtx) {
	const uint32_t WIDTH = this->pathtracerConfig.GetResolution().x, HEIGHT = this->pathtracerConfig.GetResolution().y;
...
void SPIRV::sync(const SyncContext&) const {}

double SPIRV::queryDispatchTime(uint32_t frameIdx, float deviceTimestampPeriod) const {
...
void SPIRV::getBackendAccOutImgPixels(std::vector<glm::vec4>& pixels) {
	const uint32_t WIDTH = this->pathtracerConfig.GetResolution().x, HEIGHT = this->pathtracerConfig.GetResolution().y;
