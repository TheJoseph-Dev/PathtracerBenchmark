...
VkCommandBuffer CUDA::beginSingleTimeCommands() const {
...
void CUDA::endSingleTimeCommands(VkCommandBuffer commandBuffer) const {
...
void CUDA::create2DNearestImageSampler() {
...
void CUDA::createImage2D(uint32_t width, uint32_t height) {
...
void CUDA::createPathtracerImages() {
...
void CUDA::createDeviceMemory(void** dst, const void* src, size_t size) {
	if (!size) {
		*dst = nullptr;
		return;
	}
...
void CUDA::dispatchAfterGraphicsSubmit(const DispatchConext& dispatchCtx) {
	dispatch(dispatchCtx);
}

VkImageLayout CUDA::getFragmentSampledImageLayout() const {
